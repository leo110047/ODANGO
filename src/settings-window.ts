...
async function renderPetList(pets: PetState[]): Promise<void> {
  ...
  if (config.selectedPetIds.length > 0) {
    selectedIds = config.selectedPetIds;
  } else {
    // 如果沒有選擇，預設選取 isActive 的寵物，並同步到設定中
    selectedIds = pets.filter(p => p.isActive).map(p => p.odangoId);
    if (selectedIds.length > 0) {
      config.selectedPetIds = selectedIds;
      saveSelectedPetIds(selectedIds).catch(err => console.error('Failed to save default selected pets:', err));
    }
  }
  ...
  elements.petList.innerHTML = pets.map(pet => {
    ...
    const displayName = pet.stageName || pet.species;
    const settings = getPetSettings(config.petSettings, pet.odangoId);
    const isEgg = pet.stage === 'egg';

    // 蛋不能移動，所以不顯示走動設定面板
    const settingsHtml = isEgg ? '' : `
        <div class="pet-item-settings ${isSelected ? '' : 'hidden'}">
          <div class="pet-setting-row">
            <span>走動</span>
            <label class="toggle-switch small">
              <input type="checkbox" class="pet-movement-toggle" ${settings.movementEnabled ? 'checked' : ''} />
          ...
          <div class="pet-setting-row">
            ...
            <div class="slider-container small">
              <input type="range" class="pet-speed-slider" min="0.3" max="2.0" step="0.1" value="${settings.movementSpeed}" />
              <span class="pet-speed-value">${settings.movementSpeed.toFixed(1)}x</span>
            </div>
          </div>
        </div>
    `;
    ...
    return `
      <div class="pet-item ${isSelected ? 'selected' : ''}" data-pet-id="${pet.odangoId}" data-sprite-path="${pet.spritePath}" data-is-egg="${isEgg}">
  ...
  petItems.forEach(item => {
    ...
    const isEgg = (item as HTMLElement).dataset.isEgg === 'true';
    const checkbox = item.querySelector('.pet-item-checkbox') as HTMLInputElement;
    const settingsPanel = item.querySelector('.pet-item-settings') as HTMLElement | null;
    const movementToggle = item.querySelector('.pet-movement-toggle') as HTMLInputElement | null;
    const speedSlider = item.querySelector('.pet-speed-slider') as HTMLInputElement | null;
    const speedValue = item.querySelector('.pet-speed-value') as HTMLElement | null;

    // 點擊 header 區域切換選取
    const header = item.querySelector('.pet-item-header') as HTMLElement;
    header.addEventListener('click', (e) => {
      // 如果點擊的不是 checkbox 本身
      if (!(e.target as HTMLElement).classList.contains('pet-item-checkbox')) {
        checkbox.checked = !checkbox.checked;
      }
      item.classList.toggle('selected', checkbox.checked);
      // 蛋沒有設定面板
      if (settingsPanel) {
        settingsPanel.classList.toggle('hidden', !checkbox.checked);
      }
      updateSelectedPets();
    });
    ...
    // 移動開關
    if (movementToggle) {
      movementToggle.addEventListener('click', (e) => e.stopPropagation());
      movementToggle.addEventListener('change', async () => {
        await updatePetSetting(petId, { movementEnabled: movementToggle.checked });
      });
    }
    ...
    if (speedSlider && speedValue) {
      ...
      speedSlider.addEventListener('change', async () => {
        await updatePetSetting(petId, { movementSpeed: parseFloat(speedSlider.value) });
      });
    }
  });
 ...
 * 更新單隻寵物的設定
 */
async function updatePetSetting(petId: string, updates: Partial<PetDisplaySettings>): Promise<void> {
  const currentSettings = getPetSettings(config.petSettings, petId);
  const newSettings = { ...currentSettings, ...updates };
  config.petSettings[petId] = newSettings;
  await savePetSettings(config.petSettings);
  // 通知主視窗
  await emitTo('main', 'pet-settings-changed', { petId, settings: newSettings });
}
